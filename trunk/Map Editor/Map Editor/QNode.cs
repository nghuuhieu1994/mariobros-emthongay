...
namespace Map_Editor
{
    ...
    public class QGameObject
    {
        ...
        public QGameObject(int ID, int type, Rectangle bound, bool isNonStatic)
        {
            ...
            if (!isNonStatic) // static
            {
                this.rangeMovement = bound;
            }
            else
            {
                this.rangeMovement = new Rectangle(bound.X - MAX_RANGE_OF_MOVEMENT, bound.Y - MAX_RANGE_OF_MOVEMENT, MAX_RANGE_OF_MOVEMENT, MAX_RANGE_OF_MOVEMENT * 2);
            }
        }
    }
}
