...
{link=Event.findElement(e,'a');id=link.id.replace('unhide_post_','');if($('comment_id_'+id).select('.comment_content')[1])
{elem=$('comment_id_'+id).select('.comment_content')[1];new Effect.Parallel([new Effect.BlindDown(elem),new Effect.Appear(elem)],{duration:0.5});}
